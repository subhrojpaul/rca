...
function saveLoadNext(qid,tsid,stype,ansid) {
	...
	if ($('#ta-option-1').find('.cd-ta-ans-opt-img').attr('src') && !$('#cd-bkp-imgs').find('#ansimgs-'+qid).length) {
			$('#cd-bkp-imgs').append(
				'<div id="ansimgs-'+qid+'">'
					+'<img src="'+$('#ta-option-1').find('.cd-ta-ans-opt-img').attr('src')+'">'
					+'<img src="'+$('#ta-option-2').find('.cd-ta-ans-opt-img').attr('src')+'">'
					+'<img src="'+$('#ta-option-3').find('.cd-ta-ans-opt-img').attr('src')+'">'
					+'<img src="'+$('#ta-option-4').find('.cd-ta-ans-opt-img').attr('src')+'">'
					+'</div>'
			);
	}
	...
	$('#ta-option-1').find('.cd-ta-ans-opt-img').attr('src','');
	$('#ta-option-2').find('.cd-ta-ans-opt-img').attr('src','');
	$('#ta-option-3').find('.cd-ta-ans-opt-img').attr('src','');
	$('#ta-option-4').find('.cd-ta-ans-opt-img').attr('src','');
...
function renderQuestionData(qid,data) {
	//var imgbase="../images/";
	//guru test app hard codes image path fix
	var imgbase='';
	...
	$('#ta-option-1').find('.cd-ta-ans-opt-img').attr('src',imgbase+a_qstn[5]);
	$('#ta-option-2').data("ansid",a_qstn[6]);
	//$('#ta-option-2').find('img').attr('src',imgbase+a_qstn[7]);
	$('#ta-option-2').find('.cd-ta-ans-opt-img').attr('src',imgbase+a_qstn[7]);
	$('#ta-option-3').data('ansid',a_qstn[8]);
	//$('#ta-option-3').find('img').attr('src',imgbase+a_qstn[9]);
	$('#ta-option-3').find('.cd-ta-ans-opt-img').attr('src',imgbase+a_qstn[9]);
	$('#ta-option-4').data('ansid',a_qstn[10]);
	//$('#ta-option-4').find('img').attr('src',imgbase+a_qstn[11]);
	$('#ta-option-4').find('.cd-ta-ans-opt-img').attr('src',imgbase+a_qstn[11]);
...
function bufferNext(qid) {
	if (!($('#cd-bkp-imgs').find('#ansimgs-'+qid)).length) {
		...
		var nqid=0;
		for (i=0;i<$('.cd-ta-qstn-plte > .cd-ta-qplgnd-block').length;i++) {
			var e=$('.cd-ta-qstn-plte > .cd-ta-qplgnd-block:eq('+i+')');
...
function processSubmit() {
	//guru 17 Oct, fixed huge bug, confirm dialog needs to be in submit-btn here..
	// moved the confim box code
	...
	l_req.done(function( data ) {
			...
			if (window.opener && !window.opener.closed) {
				...
				window.opener.location.href="../pages/cdtamainrep.php?tsid="+$('#ta-qna').data('tsid');
			} else {
				params  = 'width='+screen.width;
				params += ', height='+screen.height;
				params += ', top=0, left=0'
				params += ',menubar,resizable,scrollbars,status,location';
				window.open("../pages/cdtamainrep.php?tsid="+$('#ta-qna').data('tsid'),"_blank",params);
			}
			close();
	});
}
...
$(document).ready(function() {
	...
	$("#ta-inst-btn").click(function(){
		...
		$("#ta-profile-btn").removeClass('sel');
		$("#ta-qpaper-btn").removeClass('sel');
	});
	
	$("#ta-profile-btn").click(function(){
		$('#ta-prof').show();
		$('#ta-inst').hide();
	    $('#ta-qna').hide();
       	$('#ta-qp').hide();
		$("#ta-profile-btn").addClass('sel');
	...
	$("#ta-prof-back-btn").click(function(){
		$('#ta-prof').hide();
	    $('#ta-qna').show();
		$("#ta-profile-btn").removeClass('sel');
	});	
	...
	$(".cd-ta-qstn-plte > .cd-ta-qplgnd-block").click(function(){
		if ($('#ta-qp').css('display')=='block') {
			$('#ta-qp').hide();
			$("#ta-qpaper-btn").removeClass('sel');
		}
		if ($('#ta-inst').css('display')=='block') {
			$('#ta-inst').hide();
			$("#ta-inst-btn").removeClass('sel');
		}
		if ($('#ta-qna').css('display')=='none') $('#ta-qna').show();
	...
	$('#ta-submit-btn').click(function(){
		// guru 17 Oct
		//guru20Jul, adding confirmation dialog and processing only if response is yes..
		x=confirm("You are about to submit your test, please press OK to continue and Cancel to return to your test");
		if(x) {
			clearInterval(timerIntervalId);
			processSubmit();
	...
	$('.cd-ta-section').hover(function(){
		$(this).find('.cd-ta-sec-dtls').css('display','block');
		$(this).find('.cd-ta-sec-dtls').css('cursor','url("../assets/css/BLACK.cur"), auto');
	},function(){
		$(this).find('.cd-ta-sec-dtls').css('display','none');
		$(this).find('.cd-ta-sec-dtls').css('cursor','pointer');
	});
