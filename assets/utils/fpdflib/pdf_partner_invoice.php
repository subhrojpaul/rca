...
function FancyTable($header,$data) {
	...
	// $this->SetFont('','B');
	//Header
	$w=array(20,20,19,21,15,15,20,20,18,20);
	$i = 0;
	$fill=false;
	$x0=$x = $this->GetX();
	$y = $this->GetY();
...
function ImprovedTable($header, $data) {
	// Column widths
	$w = array(20, 30, 19, 21, 25, 25, 25, 25, 23, 25);
	// Header
...
$header[0] = array("Order ".$pdf->Ln()." No.", 'Email ID', 'Txn type', 'Txn date',"Pkg".$pdf->Ln()." Name",'List Price (A)','Price Benefit and Taxes (B)','Net Revenue (N=A-B)',
...
// the last page prints the total etc.. we need to handle cases where all data is in page 1 and others where page 2 onwards has 5 rows or more.
// if page 1 has 4 or more rows and page 2 has 6 or more rows then new page is required. To have a common formula, do a start delta of 2 for page 1
$start_delta = 2;
$rows_printed_latest_pg = 0;
$rows_first_page = 5;

foreach($get_invoice_pdf_other_dtls_res as $key=>$dtls_val){
	if($key < $rows_first_page){
		$rows_printed_latest_pg ++;
...
// guru 28 oct, print first page table.
$pdf->FancyTable($header,$data);

$rows_per_page = 7;
$no_of_pages = ceil(($count_other_dtls - $rows_first_page)/$rows_per_page);
echo "Total rows: ", $count_other_dtls, " No of pages: ", $no_of_pages, "<br>";

for ($pages=0; $pages < $no_of_pages; $pages++) { 

	// now we are in pages > 1 so, we are printing regular rows_per_page so no need of start_delta
	$start_delta = 0;
	$rows_printed_latest_pg = 0;
	// for each page, prep the data table.
	echo "currently in page: ", $pages, "<br>";
	$data2 = array();
	for ($pointer=($rows_first_page+($pages*$rows_per_page)); $pointer < ($rows_per_page+($rows_first_page+($pages*$rows_per_page)))&& $pointer < ($count_other_dtls) ; $pointer++) { 
		$rows_printed_latest_pg++;
		echo "page: ", $pages, " row: ", $pointer,"<br>";
		$dtls_val = $get_invoice_pdf_other_dtls_res[$pointer];
		$price_benefit = $dtls_val['transaction_tax'] - $dtls_val['transaction_discount'];
		$net_revenue = $dtls_val['transaction_price'] - $dtls_val['transaction_tax'];
		$revenue_share = $dtls_val['value']*100;
...
// if the last row has more than 5 rows then add page
$last_pg_factor = ($rows_printed_latest_pg)%$rows_per_page;
echo "last page factor (total rows - rows in pg1) %(mod) rows per page : ", $last_pg_factor, "<br>";
if($last_pg_factor=0 || $last_pg_factor > ($rows_per_page-2)) {
	echo "adding a last page..", "<br>";
	$pdf->AddPage();
}
...
//guru 28 oct, all this needs to move up..
/*
// First page
$pdf->AddPage();
$pdf->SetFont('Arial','',10);
